...
// ── VIDEO CARD ────────────────────────────────────────────────────
function VideoCard({ v, onClick, onDelete, isAdmin, C }) {
    ...
    >
      ...
      <div style={{position:"relative",paddingBottom:"56.25%",background:"#000"}}>
        ...
        <div style={{position:"absolute",top:10,left:10,display:"flex",alignItems:"center",gap:4}}>
          <div style={{background:C.accent,color:"#fff",fontFamily:"'DM Mono',monospace",fontWeight:700,fontSize:14,padding:"3px 10px",borderRadius:6,boxShadow:"0 2px 8px rgba(0,0,0,.4)"}}>
            {v.linea}
          </div>
        </div>
        {v.tipo==="deviazione" && <div style={{position:"absolute",top:10,right:10}}><Badge tipo="deviazione" small C={C} /></div>}
        {isAdmin && (
      ...
      <div style={{padding:"13px 15px 15px"}}>
        {v.tipo==="normale" && <div style={{marginBottom:6}}><Badge tipo="normale" small C={C} /></div>}
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",fontSize:11,color:C.textDim}}>
...
function VideoModal({ v, onClose, C }) {
  ...
  return (
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.88)",zIndex:900,display:"flex",alignItems:"center",justifyContent:"center",padding:16,backdropFilter:"blur(8px)"}}>
      <div onClick={e=>e.stopPropagation()} style={{background:C.card,borderRadius:18,width:"100%",maxWidth:960,border:`1px solid ${C.borderHi}`,overflow:"hidden",boxShadow:"0 32px 100px rgba(0,0,0,.8)",display:"flex",flexDirection:"column",maxHeight:"92vh"}}>
        {/* Header strip */}
...
// ── ADD VIDEO MODAL ───────────────────────────────────────────────
function AddModal({ user, onClose, onAdd, C }) {
  ...
  return (
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.88)",zIndex:900,display:"flex",alignItems:"center",justifyContent:"center",padding:16,backdropFilter:"blur(8px)"}}>
      <div onClick={e=>e.stopPropagation()} style={{background:C.card,borderRadius:18,width:"100%",maxWidth:520,border:`1px solid ${C.borderHi}`,padding:28,boxShadow:"0 32px 100px rgba(0,0,0,.8)",maxHeight:"90vh",overflowY:"auto"}}>
...
function GuideModal({ onClose, C }) {
  const [step,setStep] = useState(0);
  const s = STEPS[step];
  return (
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.88)",zIndex:900,display:"flex",alignItems:"center",justifyContent:"center",padding:16,backdropFilter:"blur(8px)"}}>
      <div onClick={e=>e.stopPropagation()} style={{background:C.card,borderRadius:18,width:"100%",maxWidth:500,border:`1px solid ${C.borderHi}`,overflow:"hidden",boxShadow:"0 32px 100px rgba(0,0,0,.8)"}}>
        {/* Header */}
...
// ── ADMIN PANEL ───────────────────────────────────────────────────
function AdminPanel({ onClose, admins, onAddAdmin, onRemoveAdmin, C }) {
  ...
  return (
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.88)",zIndex:900,display:"flex",alignItems:"center",justifyContent:"center",padding:16,backdropFilter:"blur(8px)"}}>
      <div onClick={e=>e.stopPropagation()} style={{background:C.card,borderRadius:18,width:"100%",maxWidth:500,border:`1px solid ${C.borderHi}`,padding:28,boxShadow:"0 32px 100px rgba(0,0,0,.8)",maxHeight:"85vh",overflowY:"auto"}}>
...
// ── LOGIN SCREEN ──────────────────────────────────────────────────
function LoginScreen({ admins, allUsers, onLogin, C }) {
...
function App() {
  ...
  const [preferite,setPreferite] = useState(()=>{
    try { return JSON.parse(localStorage.getItem("linee_preferite")||"[]"); } catch{ return []; }
  });
  const [showPreferite,setShowPref] = useState(false);
  const [showMenu,setShowMenu] = useState(false);
  C = THEMES[theme];

  const togglePreferita = (linea) => {
    setPreferite(p => {
      const nuove = p.includes(linea) ? p.filter(l=>l!==linea) : [...p, linea];
      localStorage.setItem("linee_preferite", JSON.stringify(nuove));
      return nuove;
    });
  };
  ...
  if (!user) return <LoginScreen admins={admins} onLogin={setUser} C={C} />;
  ...
  return (
    <>
      ...
      <div style={{minHeight:"100vh",background:C.bg,color:C.text,transition:"background .3s,color .3s",overflowX:"hidden"}}>
        ...
        <header style={{background:C.surface,borderBottom:`1px solid ${C.border}`,position:"sticky",top:0,zIndex:100,transition:"background .3s"}}>
          <div style={{maxWidth:1140,margin:"0 auto",padding:"0 20px",display:"flex",alignItems:"center",justifyContent:"space-between",height:62}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:8,position:"relative"}}>
              {devCount>0&&<div style={{background:C.dangerDim,border:`1px solid rgba(229,62,62,.3)`,borderRadius:20,padding:"3px 9px",fontSize:11,color:"#fc8181",fontWeight:700}}>⚠{devCount}</div>}
              {user.isAdmin&&<button onClick={()=>setAdd(true)} style={{background:C.accent,border:"none",color:"#fff",padding:"7px 14px",borderRadius:8,cursor:"pointer",fontSize:13,fontWeight:800,whiteSpace:"nowrap",boxShadow:`0 4px 14px ${C.accentGlow}`}}>+ Video</button>}
              <button onClick={()=>setShowMenu(m=>!m)} style={{background:C.surface,border:`1px solid ${C.border}`,color:C.textMid,padding:"7px 12px",borderRadius:8,cursor:"pointer",fontSize:20,lineHeight:1,letterSpacing:2}}>···</button>
              {showMenu&&<div onClick={()=>setShowMenu(false)} style={{position:"fixed",inset:0,zIndex:199}}/>}
              {showMenu&&(
                <div style={{position:"absolute",top:"calc(100% + 8px)",right:0,background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:8,zIndex:200,boxShadow:"0 8px 32px rgba(0,0,0,0.3)",minWidth:185}}>
                  {[
                    {icon:"⭐",label:`Preferite${preferite.length>0?" ("+preferite.length+")":""}`,action:()=>{setShowPref(p=>!p);setShowMenu(false);}},
                    {icon:theme==="dark"?"☀️":"🌙",label:theme==="dark"?"Tema chiaro":"Tema scuro",action:()=>{setTheme(t=>t==="dark"?"light":"dark");setShowMenu(false);}},
                    {icon:"📖",label:"Guida",action:()=>{setGuide(true);setShowMenu(false);}},
                    ...(user.isAdmin?[{icon:"⚙️",label:"Pannello Admin",action:()=>{setAdminP(true);setShowMenu(false);}}]:[]),
                    {icon:"⎋",label:"Esci",action:()=>{setUser(null);setShowMenu(false);},danger:true},
                  ].map((item,i)=>(
                    <button key={i} onClick={item.action} style={{display:"flex",alignItems:"center",gap:10,width:"100%",padding:"10px 14px",borderRadius:8,cursor:"pointer",background:"transparent",border:"none",color:item.danger?"#fc8181":C.text,fontSize:13,fontWeight:600,textAlign:"left"}}
        ...
        <main style={{maxWidth:1140,margin:"0 auto",padding:"24px 20px"}}>
            ...
            <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
              {linee.map(l=>(
                <div key={l} style={{display:"flex",alignItems:"center",gap:2}}>
                  ...
                  {l!=="TUTTE" && (
                    <button onClick={()=>togglePreferita(l)} title={preferite.includes(l)?"Rimuovi dai preferiti":"Aggiungi ai preferiti"}
                      style={{background:"transparent",border:"none",cursor:"pointer",fontSize:14,opacity:preferite.includes(l)?1:0.3,padding:"2px 4px",transition:"opacity .15s"}}
                    >{preferite.includes(l)?"⭐":"☆"}</button>
                  )}
                </div>
              ))}
            </div>
          ...
          {/* ── LINEE PREFERITE ────────────────────────────────── */}
          {showPreferite && (
            <div style={{background:C.card,border:`1px solid ${C.borderHi}`,borderRadius:14,padding:"16px 20px",marginBottom:20}}>
              <div style={{fontSize:11,color:C.textMid,fontWeight:700,letterSpacing:"0.07em",marginBottom:12}}>⭐ LE TUE LINEE PREFERITE</div>
              {preferite.length===0 ? (
                <div style={{fontSize:13,color:C.textDim,fontStyle:"italic"}}>
                  Nessuna linea preferita ancora. Clicca ⭐ accanto a una linea per aggiungerla.
                </div>
              ) : (
                <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:12}}>
                  {preferite.map(l=>(
                    <div key={l} style={{display:"flex",alignItems:"center",gap:6,background:C.accent,borderRadius:8,padding:"5px 12px"}}>
                      <span style={{color:"#fff",fontWeight:700,fontSize:13}}>Linea {l}</span>
                      <button onClick={()=>togglePreferita(l)} style={{background:"rgba(0,0,0,0.2)",border:"none",color:"#fff",borderRadius:4,width:18,height:18,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,padding:0}}>✕</button>
                    </div>
                  ))}
                </div>
              )}
              <div style={{fontSize:11,color:C.textDim,marginTop:8}}>
                Aggiungi linee dai pulsanti filtro qui sotto — clicca ⭐ accanto al nome della linea.
              </div>
            </div>
          )}
            ...
            : <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(285px,1fr))",gap:16}}>
                {filtered.map((v,i)=>(
                  <div key={v.id} className="card-animate" style={{animationDelay:`${i*40}ms`}}>
                    <VideoCard v={v} onClick={setSel} onDelete={handleDelete} isAdmin={user.isAdmin} C={C} />
                  </div>
                ))}
              </div>
          }
      ...
      {selVideo  && <VideoModal v={selVideo} onClose={()=>setSel(null)} C={C} />}
      {showAdd   && <AddModal user={user} onClose={()=>setAdd(false)} onAdd={handleAdd} C={C} />}
      {showGuide && <GuideModal onClose={()=>setGuide(false)} C={C} />}
      {showAdmin && <AdminPanel onClose={()=>setAdminP(false)} admins={admins} onAddAdmin={handleAddAdmin} onRemoveAdmin={handleRemoveAdmin} C={C} />}
    </>
  );
}
