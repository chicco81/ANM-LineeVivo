    ...
    <>
      ...
      <div style={{minHeight:"100vh",background:C.bg,color:C.text,transition:"background .3s,color .3s",overflowX:"hidden"}}>
        ...
        <header style={{background:C.surface,borderBottom:`1px solid ${C.border}`,position:"sticky",top:0,zIndex:100,transition:"background .3s"}}>
          <div style={{maxWidth:1140,margin:"0 auto",padding:"0 20px",display:"flex",alignItems:"center",justifyContent:"space-between",minHeight:62,flexWrap:"wrap",gap:8,padding:"8px 0"}}>
            {/* Brand */}
            ...
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              ...
              <button onClick={()=>setShowPref(p=>!p)} style={{background:showPreferite?C.accent:C.surface,border:`1px solid ${showPreferite?C.accent:C.border}`,color:showPreferite?"#fff":C.textMid,padding:"6px 13px",borderRadius:8,cursor:"pointer",fontSize:12,fontWeight:600}}>⭐ Preferite {preferite.length>0?`(${preferite.length})`:""}</button>
